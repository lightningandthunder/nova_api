...
"""
A threadsafe version of the SwissephLib class. Runs about 13.6 times slower. Every single call to the swisseph DLL
is wrapped in a thread, each running sequentially.
Not recommended unless race conditions with the underlying swisseph DLL are encountered.
"""


class SwissephLibThreadSafe:
    ...
    def _run_thread(self, func, args):
        ...
        results = thread.join()
        if thread.is_alive():
            print(f"Thread still alive after join() returned: {func, *args}")
        return results
